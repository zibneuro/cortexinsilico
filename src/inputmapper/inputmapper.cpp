...
SynapseList generateSynapses(const SegmentList& segments,
                             ...
                             const SparseField& innervation,
                             const CIS3D::Structure structure,
                             const int presynapticId,
                             const Vec3f preSoma)
{
    SynapseList result;

    QMap<Vec3i, int> numSynapsesPerVoxel;
    for (SparseField::ConstIterator it=innervation.constBegin(); it!=innervation.constEnd(); ++it) {
    ...
    // Create list of segments of the right structure passing through each voxel that contains synapses
    ...
    for (QMap<Vec3i,int>::ConstIterator it=numSynapsesPerVoxel.constBegin(); it!=numSynapsesPerVoxel.constEnd(); ++it) {
        const Vec3i& location = it.key();
        const int numSynapses = it.value();

        const QList<int> segmentIds = segmentsPerVoxel.value(location);
        if (segmentIds.size() == 0) {
            qStdErr() << "[-] WARNING: no segments for voxel" << location.getX() << location.getY() << location.getZ() << "\n";
        }
        ...
        for (int n=0; n<numSynapses; ++n) {
            ...
            for(int i=0; i<cumulativeLength.size(); ++i) {
                if (cumulativeLength[i] >= targetLength) {
                    ...
                    const float alpha = fabs(synSeg.length) > 1.e-4 ? (targetLength - start)/synSeg.length : 0.0f;
                    ...
                    s.sectionX = synSeg.p0.sectionX + alpha * (synSeg.p1.sectionX - synSeg.p0.sectionX);
                    s.distanceToSomaPost = synSeg.p0.distanceToSoma + alpha * (synSeg.p1.distanceToSoma - synSeg.p0.distanceToSoma);
                    ...
                    if (s.sectionX < 0.0 || s.sectionX > 1.0) {
                        qStdErr() << "[-] WARNING: invalid location on section:" << synSeg.sectionID << "Location: " << s.sectionX
                                  << "p0:" << synSeg.p0.sectionX << "p1:" << synSeg.p1.sectionX << "alpha:" << alpha << "\n";
                    }
                    break;
                }
            }
...
SparseField computeLengthField(const SegmentList& segments,
                               const CIS3D::Structure structure,
...
SparseField computeAreaField(const SegmentList& segments,
                             const CIS3D::Structure structure,
                             const SparseFieldCoordinates& coords)
{
    SparseField field(coords);
...
SparseField computeInnervation(const SparseField& psts,
...
void writeSynConFiles(const SynapseList& synapses,
                      const NetworkProps& network,
                      const QString& hocFileName,
                      const QString& outputDir)
{
    ...
    for (SynapseList::ConstIterator it=synapses.constBegin(); it!=synapses.constEnd(); ++it) {
        ...
        QMap<int, SynapseSectionPropsList>::iterator preIdIt = preNeuronsPerCtReg.find(syn.preNeuronId);
    ...
    if (!conFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        const QString msg = QString("Error saving .con file. Could not open file %1").arg(conFileFullPath);
        throw std::runtime_error(qPrintable(msg));
    }

    if (!synFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        const QString msg = QString("Error saving .syn file. Could not open file %1").arg(synFileFullPath);
        throw std::runtime_error(qPrintable(msg));
    }
         ...
         it!=sectionInfosSortedByCtCol.constEnd(); ++it)
    {
             ...
             preIdIt!=sortedByPreId.constEnd(); ++preIdIt)
        {
            ...
            for (int i=0; i<synapseSectionPropsList.size(); ++i) {
                ...
                synOut << ctReg << sep << synapseSectionPropsList[i].sectionID << sep << synapseSectionPropsList[i].sectionX << "\n";
                ++synIdCon;
            }
            ++preIdCon;
        }
...
void writeCSVFile(const SynapseList& synapses,
                  ...
                  const QString& outputDir)
{
    ...
    if (!csv.open(QIODevice::WriteOnly | QIODevice::Text)) {
        const QString msg = QString("Error saving .csv file. Could not open file %1").arg(csvFileFullPath);
        throw std::runtime_error(qPrintable(msg));
    }
    ...
    out << "ID" << sep
        ...
        << "Y" << sep
        << "Z" << sep
        << "Presynaptic cellID" << sep
        << "Presynaptic soma X" << sep
        << "Presynaptic soma Y" << sep
        << "Presynaptic soma Z" << sep
        << "Presynaptic cell type" << sep
        << "Presynaptic region" << sep
        << "Euclidean distance to presynaptic soma" << sep
        << "Distance to postsynaptic soma along morphology" << sep
        << "SectionID" << sep
        << "SectionName" << sep
        << "SectionX" << sep
        << "Structure" << "\n";
    ...
    for (int s=0; s<synapses.size(); ++s) {
        ...
        out << syn.id << sep
            ...
            << networkProps.neurons.getSomaPosition(syn.preNeuronId).getZ() << sep
            << preCtName << sep
            << preRegionName << sep
            << syn.euclideanDistanceToSomaPre << sep
            << syn.distanceToSomaPost << sep
            << syn.sectionID << sep
            << sectionName << sep
            << syn.sectionX << sep
            << CIS3D::getStructureName(syn.structure) << "\n";
    }
}


void writeCSVSummaryFile(const SynapseList& synapses,
                         const NetworkProps& networkProps,
                         const QString& hocFileName,
                         const QString& outputDir)
{
    ...
    QFile csv(csvFileFullPath);
    if (!csv.open(QIODevice::WriteOnly | QIODevice::Text)) {
        const QString msg = QString("Error saving summary .csv file. Could not open file %1").arg(csvFileFullPath);
        throw std::runtime_error(qPrintable(msg));
    }
    ...
    QString columnHeaders = "Number of synapses" + sep
                  + "Mean path length to soma" + sep
                  + "SD path length to soma" + sep
                  + "Connected presynaptic cells" + sep
                  + "Total presynaptic cells" + sep
                  + "Convergence" + sep

                  + "Number of synapses (apical)" + sep
                  + "Mean path length to soma (apical)" + sep
                  + "SD path length to soma (apical)" + sep
                  + "Connected presynaptic cells (apical)" + sep
                  + "Convergence (apical)" + sep

                  + "Number of synapses (basal)" + sep
                  + "Mean path length to soma (basal)" + sep
                  + "SD path length to soma (basal)" + sep
                  + "Connected presynaptic cells (basal)" + sep
                  + "Convergence (basal)" + sep

                  + "Number of synapses (soma)" + sep
                  + "Mean path length to soma (soma)" + sep
                  + "SD path length to soma (soma)" + sep
                  + "Connected presynaptic cells (soma)" + sep
                  + "Convergence (soma)" + "\n";

    out << "Presynaptic type" << sep
        << columnHeaders;

    for (QMap<int, SynapseStatistics>::ConstIterator it=statsPerCellType.constBegin(); it!=statsPerCellType.constEnd(); ++it) {
        ...
        const int totalNumPresynapticNeurons = networkProps.neurons.getFilteredNeuronIds(cellTypesToSelect, QList<int>(), CIS3D::PRESYNAPTIC).size();
        ...
        if (totalNumPresynapticNeurons > 0) {
            convergence       = float(stats.getNumberOfConnectedNeurons())/float(totalNumPresynapticNeurons);
            convergenceApical = float(stats.getNumberOfConnectedNeurons(CIS3D::APICAL))/float(totalNumPresynapticNeurons);
            convergenceBasal  = float(stats.getNumberOfConnectedNeurons(CIS3D::DEND))/float(totalNumPresynapticNeurons);
            convergenceSoma   = float(stats.getNumberOfConnectedNeurons(CIS3D::SOMA))/float(totalNumPresynapticNeurons);
        }

        out << preCtName << sep

            <<  stats.getNumberOfSynapses() << sep
            <<  stats.getDistanceToSomaStatistics().getMean() << sep
            <<  stats.getDistanceToSomaStatistics().getStandardDeviation() << sep
            <<  stats.getNumberOfConnectedNeurons() << sep
            <<  totalNumPresynapticNeurons << sep
            <<  convergence << sep
            ...
            <<  stats.getNumberOfConnectedNeurons(CIS3D::APICAL) << sep
            <<  convergenceApical << sep
            ...
            <<  stats.getNumberOfConnectedNeurons(CIS3D::DEND) << sep
            <<  convergenceBasal << sep
            ...
            <<  stats.getNumberOfConnectedNeurons(CIS3D::SOMA) << sep
            <<  convergenceSoma << "\n";
    }
    ...
    for (QMap<CIS3D::CellTypeRegion, SynapseStatistics>::ConstIterator it=statsPerCellTypeRegion.constBegin();
         it!=statsPerCellTypeRegion.constEnd(); ++it)
    {
        ...
        const int totalNumPresynapticNeurons = networkProps.neurons.getFilteredNeuronIds(cellTypesToSelect,
                                                                                         regionsToSelect,
                                                                                         CIS3D::PRESYNAPTIC).size();
...
int main(int argc, char *argv[])
{
    ...
    const float apicalLengthDensityExc = networkProps.densities.getLengthDensity(celltype, CIS3D::APICAL, CIS3D::EXCITATORY);
    const float basalLengthDensityExc  = networkProps.densities.getLengthDensity(celltype, CIS3D::DEND,   CIS3D::EXCITATORY);
    const float somaLengthDensityExc   = networkProps.densities.getLengthDensity(celltype, CIS3D::SOMA,   CIS3D::EXCITATORY);

    //const float apicalLengthDensityInh = networkProps.densities.getLengthDensity(celltype, CIS3D::APICAL, CIS3D::INHIBITORY);
    //const float basalLengthDensityInh  = networkProps.densities.getLengthDensity(celltype, CIS3D::DEND,   CIS3D::INHIBITORY);
    //const float somaLengthDensityInh   = networkProps.densities.getLengthDensity(celltype, CIS3D::SOMA,   CIS3D::INHIBITORY);

    const float apicalAreaDensityExc   = networkProps.densities.getAreaDensity  (celltype, CIS3D::APICAL, CIS3D::EXCITATORY);
    const float basalAreaDensityExc    = networkProps.densities.getAreaDensity  (celltype, CIS3D::DEND,   CIS3D::EXCITATORY);
    const float somaAreaDensityExc     = networkProps.densities.getAreaDensity  (celltype, CIS3D::SOMA,   CIS3D::EXCITATORY);

    //const float apicalAreaDensityInh   = networkProps.densities.getAreaDensity  (celltype, CIS3D::APICAL, CIS3D::INHIBITORY);
    //const float basalAreaDensityInh    = networkProps.densities.getAreaDensity  (celltype, CIS3D::DEND,   CIS3D::INHIBITORY);
    //const float somaAreaDensityInh     = networkProps.densities.getAreaDensity  (celltype, CIS3D::SOMA,   CIS3D::INHIBITORY);

    const SparseField* normalizedPSTexc = SparseField::load(CIS3D::getPSTAllFullPath(dataRoot, CIS3D::EXCITATORY));
    //const SparseField* normalizedPSTinh = SparseField::load(CIS3D::getPSTAllFullPath(dataRoot, CIS3D::INHIBITORY));
    ...
    const SparseField apicalLength = computeLengthField(segmentList, CIS3D::APICAL, segmentsGridCoords);
    const SparseField apicalArea   = computeAreaField  (segmentList, CIS3D::APICAL, segmentsGridCoords);
    const SparseField basalLength  = computeLengthField(segmentList, CIS3D::DEND,   segmentsGridCoords);
    ...
    const SparseField apicalPSTexc = apicalLength.multiply(apicalLengthDensityExc) + apicalArea.multiply(apicalAreaDensityExc);
    const SparseField basalPSTexc  = basalLength.multiply(basalLengthDensityExc)   + basalArea.multiply(basalAreaDensityExc);
    const SparseField somaPSTexc   = somaLength.multiply(somaLengthDensityExc)     + somaArea.multiply(somaAreaDensityExc);
    //const SparseField apicalPSTinh = apicalLength.multiply(apicalLengthDensityInh) + apicalArea.multiply(apicalAreaDensityInh);
    //const SparseField basalPSTinh  = basalLength.multiply(basalLengthDensityInh)   + basalArea.multiply(basalAreaDensityInh);
    //const SparseField somaPSTinh   = somaLength.multiply(somaLengthDensityInh)     + somaArea.multiply(somaAreaDensityInh);

    const QList<int> preNeuronIds = networkProps.neurons.getFilteredNeuronIds(QList<int>(), QList<int>(), CIS3D::PRESYNAPTIC);

    // Create map: (axonIdToUse)-> [preId] to process all cells with that bouton file to avoid re-reading file multiple times
    ...
    qStdOut() << "[*] Computing synapses with " << preNeuronIds.size() << " presynaptic neurons. Unique: " << idsPerAxonToUse.size() << "\n";

    SynapseList synapses;
    int processed = 0;

    for (QMap<int, QList<int>>::ConstIterator it=idsPerAxonToUse.constBegin(); it!=idsPerAxonToUse.constEnd(); ++it) {
        ...
        const SparseField* boutons = SparseField::load(CIS3D::getBoutonsFileFullPath(dataRoot, preCTname, axonIdToUse));
        ...
        const SparseField apicalInnervation = computeInnervation(apicalPSTexc, *boutons, *normalizedPSTexc);
        const SparseField basalInnervation  = computeInnervation(basalPSTexc,  *boutons, *normalizedPSTexc);
        const SparseField somaInnervation   = computeInnervation(somaPSTexc,   *boutons, *normalizedPSTexc);

        //const SparseField apicalInnervation = isExc ? computeInnervation(apicalPSTexc, *boutons, *normalizedPSTexc)
        //                                            : computeInnervation(apicalPSTinh, *boutons, *normalizedPSTinh);

        //const SparseField basalInnervation  = isExc ? computeInnervation(basalPSTexc,  *boutons, *normalizedPSTexc)
        //                                            : computeInnervation(basalPSTinh,  *boutons, *normalizedPSTinh);

        //const SparseField somaInnervation   = isExc ? computeInnervation(somaPSTexc,   *boutons, *normalizedPSTexc)
        //                                            : computeInnervation(somaPSTinh,   *boutons, *normalizedPSTinh);

        const QList<int>& preIds = it.value();

        for (QList<int>::ConstIterator idIt=preIds.constBegin(); idIt!=preIds.constEnd(); ++idIt) {
            const int preId = *idIt;
            const Vec3f preSoma = networkProps.neurons.getSomaPosition(preId);
            const SynapseList apicalSyn = generateSynapses(segmentList, segmentsGridCoords, apicalInnervation, CIS3D::APICAL, preId, preSoma);
            const SynapseList basalSyn  = generateSynapses(segmentList, segmentsGridCoords, basalInnervation,  CIS3D::DEND,   preId, preSoma);
            const SynapseList somaSyn   = generateSynapses(segmentList, segmentsGridCoords, somaInnervation,   CIS3D::SOMA,   preId, preSoma);
            ...
            if (processed % 100000 == 0) {
                qStdOut() << "    Number of presynaptic cells processed: " << processed << "\tNum synapses: " << synapses.size() << "\n";
            }
        }
