                                    ...
                                    double eps)
{
    ...
    // ########### POST ALL EXC ###########
    fprintf(stderr,"[*] Loading postsynaptic all excitatory.\n");
    QString postAllExcFile =
    ...
    // ########### POST ALL INH ###########
	fprintf(stderr, "[*] Loading postsynaptic all inhibitory.\n");
    ...
    // ########### PRE ###########
	fprintf(stderr, "[*] Loading presynaptic.");
    for (int i = 0; i < selection.Presynaptic().size(); i++)
    {
        int neuronId = selection.Presynaptic()[i];
    ...
    // ########### POST ###########
	fprintf(stderr, "[*] Loading postsynaptic.");
    for (int i = 0; i < selection.Postsynaptic().size(); i++)
    {
        int neuronId = selection.Postsynaptic()[i];
	...
	//fprintf(stderr, "[*] Number of voxels after prunig" << voxel.size();
    ...
    */
	...
	fprintf(stderr, "[*] Write postsynaptic inhibitory.");
	...
	fprintf(stderr, "[*] Write index files.");
	...
	fprintf(stderr, "[*] Finished writing features.");
}
