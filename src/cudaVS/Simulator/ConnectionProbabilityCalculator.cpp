...
ConnectionProbabilityCalculator::distributeSynapses(QVector<float> parameters)
{
	...
	fprintf(stderr, "Start distributing synapses [%f,%f,%f,%f].", b0, b1, b2, b3);
    ...
    mFeatureProvider.load(neuron_pre, b1, neuron_postExc, b2, neuron_postInh, voxel_postAllExc, b3, voxel_postAllInh, neuron_funct, voxel_neuronsPre, voxel_neuronsPostExc, voxel_neuronsPostInh);
...
//#pragma omp parallel for schedule(dynamic)
    ...
    fprintf(stderr, "Distributed synapses [%f,%f,%f,%f]. Connection prob.: %f", b0, b1, b2, b3, connProb);
    return connProb;
}
