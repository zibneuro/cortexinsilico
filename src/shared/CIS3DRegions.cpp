...
void Regions::loadCSV(const QString &fileName)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        const QString msg = QString("Error reading regions file. Could not open file %1").arg(fileName);
        throw std::runtime_error(qPrintable(msg));
    }
    ...
    if (parts.size() != 3 ||
        ...
        parts[ 2] != "ParentId")
    {
        const QString msg = QString("Error reading regions file %1. Invalid column headers.").arg(fileName);
        throw std::runtime_error(qPrintable(msg));
    }
    ...
    while (!line.isNull()) {
        ...
        if (parts.size() != 3) {
            const QString msg = QString("Error reading regions file %1. Invalid columns.").arg(fileName);
            throw std::runtime_error(qPrintable(msg));
        }
