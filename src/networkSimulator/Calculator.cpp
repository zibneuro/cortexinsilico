...
*/
Calculator::Calculator(FeatureProvider &featureProvider,
                       RandomGenerator &randomGenerator,
                       std::vector<int> runIndices)
    : mFeatureProvider(featureProvider), mRandomGenerator(randomGenerator),
      mRunIndices(runIndices) {}

void Calculator::calculateBatch(std::vector<QVector<float>> parametersBatch,
                                QString mode,
                                std::map<QString, float> &propsFloat,
                                std::map<QString, bool> &propsBoolean) {
  ...
  float innervationCutoffLog =
      innervationCutoff == 0 ? -100 : log(innervationCutoff);
  ...
  bool synapsePerVoxel = propsBoolean["SYNAPSE_PER_VOXEL"];
  ...
  mFeatureProvider.load(neuron_pre, 1, neuron_postExc, 1, neuron_postInh,
                        voxel_postAllExc, 1, voxel_postAllInh, neuron_funct,
                        voxel_neuronsPre, voxel_neuronsPostExc,
  ...
  for (unsigned int run = 0; run < mRunIndices.size(); run++) {
    ...
    constraintCount.push_back(0); // total number of checks, i.e. number of all
                                  // neuron pairs in all voxels
    constraintCount.push_back(
        0); // number of violated bouton vs. PST magnitude bounds
    ...
    if (mode == "h0_intercept_pre_pst_pstAll") {
      ...
      b3 = parameters[3];
      paramString =
          QString("Simulating h0_intercept_pre_pst_pstAll [%1,%2,%3,%4].")
              .arg(b0)
              .arg(b1)
              .arg(b2)
              .arg(b3);
      ...
      b2 = parameters[1];
      b3 = parameters[2];
      paramString = QString("Simulating h0_pre_pst_pstAll [%1,%2,%3].")
                        .arg(b1)
                        .arg(b2)
                        .arg(b3);
      ...
      b1 = parameters[1];
      b2 = parameters[2];
      paramString = QString("Simulating h0_intercept_pre_pstNorm [%1,%2,%3].")
                        .arg(b0)
                        .arg(b1)
                        .arg(b2);
      ...
      b1 = parameters[0];
      b2 = parameters[1];
      paramString = QString("Simulating h0_intercept_pre_pstNorm [%1,%2].")
                        .arg(b1)
                        .arg(b2);
    } else {
      const QString msg = QString("Invalid simulation mode: %1").arg(mode);
      throw std::runtime_error(qPrintable(msg));
    }

    // ###################### INIT OUTPUT FIELDS ######################

    std::vector<long> empty(postIndices.size(), 0);
    std::vector<float> emptyFloat(postIndices.size(), 0);
    std::vector<std::vector<long>> contacts(preIndices.size(), empty);
    ...
    for (unsigned int i = 0; i < preIndices.size(); i++) {
      ...
      for (unsigned int j = 0; j < postIndices.size(); j++) {
        ...
        if (preId != postId) {
          for (auto pre = neuron_pre[preId].begin();
               pre != neuron_pre[preId].end(); ++pre) {
            if (neuron_postExc[postId].find(pre->first) !=
                neuron_postExc[postId].end()) {
              ...
              // qDebug() << preVal << postVal << postAllVal << postNormVal <<
              // arg;

              bool orderOfMagnitudeBoundViolated =
                  checkBoutonPstRatio && (boundArg1 < boutonPSTRatio);
              bool probabilityConstraintViolated =
                  checkPstProbability && (boundArg2 > 0);
              bool innervationBoundViolated =
                  checkMaxInnervation && (arg > maxInnervationLog);

              bool anyViolated = updateConstraintCount(
                  constraintCount, orderOfMagnitudeBoundViolated,
                  probabilityConstraintViolated, innervationBoundViolated);

              arg = std::min(arg, innervationCutoffLog);
              float mu = exp(arg);
              long synapses = synapsePerVoxel
                                  ? mRandomGenerator.drawPoissonDouble(
    ...
    if (!synapsePerVoxel) {
      for (unsigned int i = 0; i < preIndices.size(); i++) {
        for (unsigned int j = 0; j < postIndices.size(); j++) {
          double lambda = static_cast<double>(innervation[i][j]);
          contacts[i][j] = mRandomGenerator.drawPoissonDouble(lambda);
        }
      }
    }
    ...
    for (unsigned int i = 0; i < preIndices.size(); i++) {
      ...
      double probability =
          (double)realizedConnections / (double)postIndices.size();
    ...
    writeStatistics(runIndex, connProbSynapse.getMean(),
                    connProbInnervation.getMean(), sufficientStat,
                    constraintCount);
    ...
    qDebug()
        << "RUN: " << runIndex << paramString
        << QString("Connection prob.: %1").arg(connProbInnervation.getMean());
  }
}

void Calculator::calculateSpatial(
    std::map<int, std::map<int, float>> &neuron_pre,
    std::map<int, std::map<int, float>> &neuron_postExc,
...
#pragma omp for
    for (unsigned int i = 0; i < preIndices.size(); i++) {
      ...
      QString fileName = QDir(innervationDir)
                             .filePath("preNeuronID_" + QString::number(preId));
      ...
      for (auto itPost = innervationPerPre.begin();
           itPost != innervationPerPre.end(); ++itPost) {
        if (itPost->second.begin() != itPost->second.end()) {
          stream << "postNeuronID"
                 << " " << itPost->first << "\n";
        }
        for (auto itVoxel = itPost->second.begin();
             itVoxel != itPost->second.end(); ++itVoxel) {
...
double Calculator::calculateProbability(double innervationMean) {
...
void Calculator::writeSynapseMatrix(int runIndex,
                                    std::vector<std::vector<long>> &contacts) {
...
void Calculator::writeInnervationMatrix(
    int runIndex, std::vector<std::vector<float>> &innervation) {
...
void Calculator::writeStatistics(int runIndex, double connectionProbability,
  ...
  out << "\"CONNECTION_PROBABILITY_INNERVATION\":"
      << connectionProbabilityInnervation << ",";
...
bool Calculator::updateConstraintCount(std::vector<long> &count,
  ...
  bool anyViolated = magnitudeBoundViolated || probabilityBoundViolated ||
                     maxInnervationBoundViolated;
  if (anyViolated) {
    count[4] += 1;
  }
  return anyViolated;
...
void Calculator::writeRawData(int runIndex, std::vector<RawDataItem> &rawData) {
  ...
  for (unsigned int i = 0; i < rawData.size(); i++) {
    ...
    out << r.beta0 << " " << r.beta1 << " " << r.beta2 << " " << r.beta3 << " "
        << r.voxelId << " " << r.preId << " " << r.postId << " " << r.pre << " "
        << r.post << " " << r.postAll << " " << r.postNorm << " "
        << r.innervation << " " << r.synapses << " " << r.magnitudeBound << " "
        << r.probabilityBound << " " << r.innervationBound << " " << r.discarded
        << "\n";
  }
}
