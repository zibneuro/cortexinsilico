...
VoxelQueryHandler::createJsonResult()
{
    ...
    qDebug() << preCellsPerVoxel.getMean();

    QJsonObject result;
    result.insert("numberInnervatedVoxels", (int)mMapPreCellsPerVoxel.size());
    ...
    result.insert("postsynapticCellsPerVoxel", Util::createJsonStatistic(mPostsynapticCellsPerVoxel));
    result.insert("postsynapticCellsPerVoxelHisto", Util::createJsonHistogram(mPostsynapticCellsPerVoxelH));
    ...
    result.insert("postsynapticSitesPerVoxel", Util::createJsonStatistic(mPostsynapticSitesPerVoxel));
    result.insert("postsynapticSitesPerVoxelHisto", Util::createJsonHistogram(mPostsynapticSitesPerVoxelH));
    result.insert("synapsesPerConnection", Util::createJsonStatistic(mSynapsesPerConnection));
    result.insert("synapsesPerConnectionHisto", Util::createJsonHistogram(mSynapsesPerConnectionH));
...
VoxelQueryHandler::replyGetQueryFinished(QNetworkReply* reply)
{
    ...
    else if (error == QNetworkReply::NoError)
    {
        ...
        QString dataFolder = QDir::cleanPath(mDataRoot + QDir::separator() + "spatialInnervation" + QDir::separator() + "features_pre");
        ...
        {
            int mappedPreId = mNetwork.axonRedundancyMap.getNeuronIdToUse(preIds[i]);
            ...
            QString dataFileName = QDir(dataFolder).filePath(QString::number(mappedPreId) + ".dat");
            ...
            while (!line.isNull())
            {
                ...
                {
                    mMapPreCellsPerVoxel[voxelId] += 1;
                }
