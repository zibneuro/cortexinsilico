...
QJsonObject VoxelQueryHandler::createJsonResult(bool createFile) {
  ...
  createStatistics(mMapPostCellsPerVoxel, postCellsPerVoxel,
                   postCellsPerVoxelH);

  Statistics postsynapticSitesPerVoxel;
  Histogram postsynapticSitesPerVoxelH(500);
  createStatistics(mMapPostsynapticSitesPerVoxel, postsynapticSitesPerVoxel,
                   postsynapticSitesPerVoxelH);
  ...
  for (auto it = mSynapsesPerConnectionOccurrences.begin();
       it != mSynapsesPerConnectionOccurrences.end(); it++) {
...
VoxelQueryHandler::VoxelQueryHandler() : QueryHandler() {}
...
void VoxelQueryHandler::doProcessQuery() {
  ...
  FeatureProvider::readMapFloat(postAllField, postAllFolder,
                                "voxel_postAllExc.dat");
...
bool VoxelQueryHandler::initSelection() { return false; };

void VoxelQueryHandler::setFilterString(QString mode, QString preFilter,
                                        QString postFilter,
                                        QString advancedSettings,
...
QString VoxelQueryHandler::getResultKey(){
    return "voxelResult";
}